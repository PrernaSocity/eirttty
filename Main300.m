...
nbus =length(busdata_300);                  % IEEE-30..
busd = busdata_300;      % Calling busdatas..
run('ybusppg300.m')
...
ET=1:240
figure,
plot(ET,Vde(1:240),'-.b*','LineWidth',1.5,'Markersize',7)
xlabel('Bus No')
ylabel('Voltage Deviation')
set(gca,'FontWeight','Bold')
title('Normal condition')
figure,
plot(ET,F1(1:240),'-.b*','LineWidth',1.5,'Markersize',7)
xlabel('Bus No')
ylabel('Voltage Deviation')
set(gca,'FontWeight','Bold')
...
figure,
plot(ET,Vde1(1:240),'-.b*','LineWidth',1.5,'Markersize',7)
xlabel('Bus No')
ylabel('Voltage Deviation')
set(gca,'FontWeight','Bold')
...
figure,
hold on;grid on
plot(ET,F4(1:240),'-.b*','LineWidth',1.5,'Markersize',7)
xlabel('Bus No')
ylabel('Voltage Deviation')
set(gca,'FontWeight','Bold')
...
set(gca,'FontWeight','Bold')
axis([1 250 0 1])
legend('Lineoutage condition','Normal condition','Generator outage condition','Both outage condition')
% Power_loss
...
Lp1=1:372
linedat118_=[linedat118(:,3);linedat118(:,3)]
linedat1180=[linedat118(:,4);linedat118(:,4)]
for i=1:length(Lp1)
    LO(i)=abs(linedat118_(i)-linedat1180(i))/i
    if LO(i)<0.1
        LO(i)=(LO(i)+0.1)*500;
    else
        LO(i)=LO(i)*1000;
...
for i=1:length(Lp1)
    GO(i)=abs(linedat118_(i)-linedat1180(i))/i
    if GO(i)<0.1
        GO(i)=(GO(i)+0.1)*2000;
    else
        GO(i)=GO(i)*2000;
...
for i=1:length(Lp1)
    BO(i)=abs(linedat118_(i)-linedat1180(i))/i
    if BO(i)<0.1
        BO(i)=(BO(i)+0.1)*1230;
    else
        BO(i)=BO(i)*1230;
