function [Pi Qi Pg Qg Pl Ql Lpij  Qij V] = loacdfolw_calc(nbus,busd,Y)
...
Iter = 1;
  % Iteration starting..
    ...
    J = J1;    % Jacobian Matrix..
    J = J(:,1);
%     X = inv(J)*TF;           % Correction Vector
    dTh = J(1:nbus-1);      % Change in Voltage Angle..
           % Change in Voltage Magnitude..
    
    % Updating State Vectors..
    del(2:nbus) =  del(2:nbus);    % Voltage Angle..
    k = 1;
    for i = 2:nbus
        if type(i) == 3
            V(i) = dTh(k) + V(i);        % Voltage Magnitude..
            k = k+1;
        end
    end
    Iter = Iter + 1;
...
[Pi Qi Pg Qg Pl Ql Lpij  Qij V ] = loadflow(nbus,V,del,BMva,busd,Y);
linedt = linedatas(nbus);
